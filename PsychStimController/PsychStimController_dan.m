...
%         end
        ...
        % darcy - backlum replaces highcontrast
        %[img cl] = generateBars_lut(handles.orient(c),handles.freq(c),handles.speed(c),handles.contrast(c),handles.length(c), handles.positionX(c),Duration, degPerPix,imageRect(3),imageRect(4),FrameHz,black,white,sizeLut,highcontrast);        
...
case 7 
    ...
    for c = 1:nCond       
        ...
        % darcy 2/21/12: added elcp and azcp compensation below
        
%          xposdeg = ((xpos-(xwinpix/2))*degPerPix);
%          yposdeg = (-1*((ypos-(ywinpix/2))*degPerPix));

%         posXpix = ((handles.positionX(c)-(xwinpix/2))*degPerPix) + azcp;
%         posYpix = ((handles.positionY(c)-(ywinpix/2))*degPerPix) + elcp;
        posXpix = (imageRect(3)/2 + handles.positionX(c)/degPerPix) + azcp;
        posYpix = (imageRect(4)/2 + handles.positionY(c)/degPerPix) + elcp;
%         
%        SquareSpots = 1;
%        if SquareSpots
...
%        else
%            for img = double ( (sqrt( ((x-posXpix)*(x-posXpix)) - ((y-posYpix)*(y-posYpix)))) < (widthPix/2) );
 %       end
        if contrast > 1
            contrast = 1;
        elseif contrast < -1
        ...
        %%% non-grey background m Stryker commented out with HighContrast
        %%% addition
%          cl(1,:) = grey-0.75*inc;
...
% darcy - backlum replaces highcontrast
% highcontrast = get(handles.HighContrast,'Value'); 
...
% darcy - backlum replaces highcontrast
% if exist('highcontrast','var')
%     set(handles.HighContrast,'Value',highcontrast);
% end
...
% --- Executes on button press in HighContrast.
function HighContrast_Callback(hObject, eventdata, handles)
% hObject    handle to HighContrast (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of HighContrast
