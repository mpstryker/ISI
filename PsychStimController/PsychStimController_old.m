...
% Last Modified by GUIDE v2.5 26-May-2010 18:05:17
...
elseif sync == kcsUDP
    ...
    % udpChannel = uint16(0);
    phMax = 65535;
 elseif sync == ktwoPhoton
    % sync via udp file name of acq file from scanimage
    syncCheck = 0;
    phMax = 65535;
    twoPhotonSyncUdp=pnet('udpsocket',8936);
    while ~syncCheck
       % Wait/Read udp packet to read buffer
        len=pnet(twoPhotonSyncUdp,'readpacket');
        if len>0
            % if packet larger then 1 byte then read maximum of 1000 doubles in network byte order
            syncfilename =[pnet(twoPhotonSyncUdp,'read') 'cond']
            startTime=GetSecs;
            syncCheck = 1;
        end
        twoPhotonSyncData = [];
...
try %% put this is a try/catch, so that any crash won't leave screen hung  
           ...
           %% loop through frames
            for f = minframe:maxframe
                ...
                elseif  sync == ktwoPhoton
                    ...
                    twoPhotonSyncData = [twoPhotonSyncData; [c ph s1(f)-startTime]];  
                    %putsample(syncTwoPhoton,5*(mod(f,phasePeriodFrames)/phasePeriodFrames));
                 elseif sync == ktdsUDP
            ...
            if sync == ktdtSync
               putvalue(parentuddobj,[bitOff bitOff],bitLine);    
            elseif  sync == ktwoPhoton
                 twoPhotonSyncData = [twoPhotonSyncData; [0 0 s1(f)-startTime]];  
            elseif sync == ktdsUDP
	...
	if sync == ktwoPhoton   
        pnet(twoPhotonSyncUdp,'close');
         %figure
         %plot(twoPhotonSyncData);
        save(syncfilename,'twoPhotonSyncData','fname');
	end
